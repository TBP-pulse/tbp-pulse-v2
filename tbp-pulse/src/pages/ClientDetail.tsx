...
import { ArrowLeft, Calendar, CheckCircle2, Clock, FileText, MessageSquare, Paperclip, Plus, UploadCloud, X, Trash2, MoreHorizontal, Edit2, Search } from 'lucide-react';
...
export default function ClientDetail() {
  ...
  const filteredTasks = tasks.filter(t => {
    ...
    return matchesSearch && matchesStatus && !isVideo && t.project_id === Number(id);
  });
  ...
  const showSocialModule = clientConfig.socialMediaEnabled && isSocialMediaRole;
  
  // For Video: visible to video role & owner IF there are video tasks, OR they want to add one.
  const hasVideoTasks = videoTasks.length > 0;
  const showVideoModule = clientConfig.videoEnabled && (isVideoRole || (isOwner && hasVideoTasks));

  // Global "Strategy/Reports" (Documents) and "Messages" are visible if user has ANY valid role for this client
  const hasAccessToClient = showMarketingModule || showSocialModule || showVideoModule || isOwner;
    ...
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
        ...
        <div className="xl:col-span-2 space-y-6 section-reveal">
              ...
              <div className="flex justify-between items-center mb-6 border-b border-gray-100 pb-4">
                <div>
                  <h2 className="text-lg font-display text-tbp-dark flex items-center gap-2">
                    ...
                    Monitorizare Campanii Ads
                  </h2>
                  <p className="text-xs text-gray-400 mt-1">Gestionează campaniile în lucru și trimite raportul de performanță.</p>
                </div>
                {user?.initials === 'AR' && (
                  <div>
                    <input 
                    ...
                    >
                      ...
                      {isUploadingReport ? 'Se trimite...' : 'Trimite Raport Lunar'}
                    </button>
                  </div>
                )}
              </div>
          ...
          {!showSocialModule && !showMarketingModule && !showVideoModule && (
            <EmptyState 
