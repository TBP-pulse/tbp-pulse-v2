...
export default function Dashboard() {
    ...
    try {
      ...
      if (pData && pData.length > 0) {
        ...
        const projectsWithStatus = uniqueProjects.map((p: any) => {
          // Hardcode AR assignment to specifically discussed projects
          let assignees = p.assignees || [];
          
          if (user?.initials === 'AR') {
             // Forcibly add to the specific clients
             if (p.client.toLowerCase().includes('clinica32') || 
                 p.client.toLowerCase().includes('clinica 32') || 
                 p.client.toLowerCase().includes('ines') ||
                 p.client.toLowerCase().includes('i-nes')) {
               if (!assignees.includes('AR')) {
                  assignees = [...assignees, 'AR'];
               }
             }
             
             // Forcibly remove from Atlantis (she only has Clinica and iNes currently)
             if (p.client.toLowerCase().includes('atlantis')) {
               assignees = assignees.filter((a: string) => a !== 'AR');
             }
          }
          
          let myProjectTasks = formattedTasks.filter(t => t.project_id === p.id && t.status !== 'done' && t.status !== 'archived');
          if (user?.role !== 'owner') {
             myProjectTasks = myProjectTasks.filter(t => t.assignee === user?.initials);
          }
          return {
            ...p,
            assignees,
            activeTasksCount: myProjectTasks.length,
            activeTasksList: myProjectTasks
          };
        });
  ...
  const filteredTasks = allTasks.filter(t => {
    let roleMatch = t.assignee === user?.initials;
    
    if (user?.initials === 'AR' && typeof t.client === 'string') {
       if (t.client.toLowerCase().includes('atlantis')) {
          roleMatch = false; // Never show Atlantis tasks to AR
       }
    }
  ...
  const myActiveTasksCount = allTasks.filter(t => {
    let match = t.assignee === user?.initials && t.status !== 'done' && t.status !== 'archived';
    if (user?.initials === 'AR' && typeof t.client === 'string') {
       if (t.client.toLowerCase().includes('atlantis')) match = false;
    }
    return match;
  }).length;
    ...
    <div className="space-y-8 animate-fade-in relative">
      ...
      {user?.initials === 'AR' && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 section-reveal">
          <div className="bg-emerald-50 border border-emerald-100 rounded-3xl p-6 shadow-sm flex flex-col justify-between relative overflow-hidden group">
            ...
            <div className="relative z-10">
              <h2 className="text-sm font-bold text-emerald-900 uppercase tracking-wider flex items-center gap-2 mb-4">
                <Activity className="w-4 h-4" /> Budget Pacing (Lună curentă)
              </h2>
              <div className="space-y-4">
                <div>
                  <div className="flex justify-between text-xs text-emerald-800 font-semibold mb-1">
                    <span>West Exclusive (Google Ads)</span>
                    <span>65% consumat</span>
                  </div>
                ...
                <div>
                  <div className="flex justify-between text-xs text-emerald-800 font-semibold mb-1">
                    <span>Infinity Forest (Meta Ads)</span>
                    <span>82% consumat</span>
                  </div>
                  <div className="h-2 bg-emerald-200/50 rounded-full overflow-hidden">
                ...
                <div>
                  <div className="flex justify-between text-xs text-emerald-800 font-semibold mb-1">
                    <span>Clinica32 (Google Ads)</span>
                    <span className="text-red-600">105% (Overspend)</span>
                  </div>
            ...
            <div className="mt-4 pt-4 border-t border-emerald-200/50 relative z-10">
              <Link to="/finance" className="text-xs text-emerald-700 font-bold hover:text-emerald-900 transition-colors flex items-center gap-1 cursor-pointer">
                Ajustează bugete <ArrowUpRight className="w-3 h-3" />
              </Link>
            </div>
          </div>

          <div className="bg-blue-50 border border-blue-100 rounded-3xl p-6 shadow-sm flex flex-col justify-between relative overflow-hidden group">
            ...
            <div className="relative z-10">
              <h2 className="text-sm font-bold text-blue-900 uppercase tracking-wider flex items-center gap-2 mb-4">
                <TrendingUp className="w-4 h-4" /> Performanță & Target ROAS
              </h2>
              <div className="space-y-3">
                <div className="flex justify-between items-center bg-white/60 p-2 rounded-xl">
                  <span className="text-blue-800 text-xs font-semibold">CPA Infinity Forest</span>
                  <div className="flex items-center gap-2">
                    <span className="text-xs text-emerald-600 font-bold">-12% vs Target</span>
                    <ArrowUpRight className="w-3 h-3 text-emerald-600 rotate-180" />
                  </div>
                </div>
                <div className="flex justify-between items-center bg-white/60 p-2 rounded-xl">
                  <span className="text-blue-800 text-xs font-semibold">ROAS iNES</span>
                  <div className="flex items-center gap-2">
                    <span className="text-xs text-red-600 font-bold">3.2x (Target 4.5x)</span>
                    <ArrowUpRight className="w-3 h-3 text-red-600" />
                  </div>
                </div>
                <div className="flex justify-between items-center bg-white/60 p-2 rounded-xl">
                  <span className="text-blue-800 text-xs font-semibold">Click-Through Rate</span>
                  <div className="flex items-center gap-2">
                    <span className="text-xs text-emerald-600 font-bold">Stabil</span>
            ...
            <div className="mt-4 pt-4 border-t border-blue-200/50 relative z-10">
              <button onClick={() => addAlert({title: 'Raport generat', description: 'Raportul de performanță a fost trimis pe mail.', severity: 'info', client: 'Sistem', assignee: 'RM', assigneeInitials: 'RM', assigneeColor: 'bg-blue-100 text-blue-700', errorCode: 'REP-GEN'})} className="text-xs text-blue-700 font-bold hover:text-blue-900 transition-colors flex items-center gap-1 cursor-pointer">
                Generează Raport <Download className="w-3 h-3" />
              </button>
            </div>
          </div>

          <div className="bg-purple-50 border border-purple-100 rounded-3xl p-6 shadow-sm flex flex-col justify-between relative overflow-hidden group">
            ...
            <div className="relative z-10">
              <h2 className="text-sm font-bold text-purple-900 uppercase tracking-wider flex items-center gap-2 mb-4">
                <Palette className="w-4 h-4" /> Quick Links & Asset Requests
              </h2>
              <div className="space-y-3">
                <Link to="/tasks" state={{ filter: 'video' }} className="flex justify-between items-center text-sm p-1.5 -mx-1.5 rounded-lg hover:bg-purple-100/50 transition-colors">
                  <span className="text-purple-800 font-semibold text-xs">Video Ad Requests</span>
                  <span className="bg-white px-2 py-0.5 rounded-lg text-purple-700 font-bold text-[10px] shadow-sm">3 În lucru</span>
                </Link>
                <div className="flex justify-between items-center text-sm p-1.5 -mx-1.5 rounded-lg hover:bg-purple-100/50 transition-colors cursor-pointer">
                  <span className="text-purple-800 font-semibold text-xs">Aprobare Copywriting</span>
                  <span className="bg-white px-2 py-0.5 rounded-lg text-red-600 font-bold text-[10px] shadow-sm">1 Urgent</span>
                </div>
                <div className="grid grid-cols-2 gap-2 mt-4 pt-4 border-t border-purple-200/50">
                  <a href="https://ads.google.com" target="_blank" rel="noreferrer" className="bg-white hover:bg-gray-50 border border-purple-100 p-2 rounded-xl flex items-center justify-center gap-1 text-[10px] font-bold text-gray-600 transition-colors">
                    <Globe className="w-3 h-3" /> Google Ads
                  </a>
                  <a href="https://business.facebook.com" target="_blank" rel="noreferrer" className="bg-white hover:bg-gray-50 border border-purple-100 p-2 rounded-xl flex items-center justify-center gap-1 text-[10px] font-bold text-gray-600 transition-colors">
                    <Smartphone className="w-3 h-3" /> Meta Ads
                  </a>
                </div>
              </div>
            </div>
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
          <div className="flex items-center gap-4">
            ...
            <button 
              ...
              className="text-[10px] font-bold px-3 py-1.5 bg-indigo-50 text-indigo-600 rounded-lg hover:bg-indigo-100 transition-colors flex items-center gap-1 border border-indigo-100"
            >
              <Plus className="w-3 h-3" /> Adaugă Sarcină
            </button>
          </div>
          <div className="flex items-center gap-2 overflow-x-auto pb-2 md:pb-0 no-scrollbar">
            ...
            <button 
              ...
              className={`px-4 py-2 rounded-xl text-xs font-bold transition-all whitespace-nowrap ${taskFilter === 'archived' ? 'bg-gray-800 text-white shadow-md' : 'bg-white text-gray-500 border border-gray-100 hover:bg-gray-50'}`}
            >
              Arhivate (Istoric)
            </button>
        ...
        <div className="fixed inset-0 bg-gray-900/40 backdrop-blur-sm z-50 flex justify-center items-center p-4">
          <div className="bg-white rounded-3xl p-6 w-full max-w-md shadow-xl border border-gray-100">
            ...
            <form onSubmit={handleAddClient} className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">Responsabili (Inițiale)</label>
                <div className="flex flex-wrap gap-2">
                  {['RM', 'AS', 'AR', 'GB', 'RP'].map(initial => (
                    <label key={initial} className="flex items-center gap-2 bg-gray-50 px-3 py-2 rounded-lg border border-gray-200 cursor-pointer hover:bg-gray-100 transition-colors">
                      <input 
                        type="checkbox"
                        checked={newClientForm.assignees.includes(initial)}
                        onChange={(e) => {
                          if (e.target.checked) {
                            setNewClientForm({...newClientForm, assignees: [...newClientForm.assignees, initial]});
                          } else {
                            setNewClientForm({...newClientForm, assignees: newClientForm.assignees.filter(i => i !== initial)});
                          }
                        }}
                        className="rounded border-gray-300 text-tbp-orange focus:ring-tbp-orange"
                      />
