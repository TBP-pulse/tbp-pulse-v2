...
export default function TaskModal({ isOpen, onClose, user, projects, defaultProjectId, onTaskAdded }: TaskModalProps) {
  ...
  // Filter assignees based on user role
  const getAllowedAssignees = () => {
    if (user?.role === 'owner') return teamMembers;
    if (user?.initials === 'AR') return teamMembers.filter(m => m.initials === 'AR' || m.initials === 'RM');
    // For other users, they can assign to themselves or Owner
    return teamMembers.filter(m => m.initials === user?.initials || m.initials === 'RM');
  };

  const allowedAssignees = getAllowedAssignees();

  // Filter projects by assignee for non-owners
  const getAllowedProjects = () => {
    if (user?.role === 'owner') return projects;
    return projects.filter(p => p.assignees?.includes(user?.initials || '') || p.client === 'General');
  };
